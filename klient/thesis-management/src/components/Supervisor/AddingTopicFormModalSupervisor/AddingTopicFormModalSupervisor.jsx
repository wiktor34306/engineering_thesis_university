...
export const AddingTopicFormModalSupervisor = ({ open, handleClose }) => {
  ...
  const handleAddTopic = () => {
    ...
    fetch(`http://localhost:3001/get-supervisor-id-by-user-id/${id_uzytkownika_dodajacego}`, {
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
        Authorization: "Bearer " + localStorage.getItem("token"),
      ...
      .then((response) => response.json())
      .then((data) => {
        // Ustaw id_promotora w stanie komponentu
    ...
    fetch("http://localhost:3001/adding-topic", {
      ...
      .then((data) => {
        ...
        if (data && data.id_tematu) {
          ...
          navigate('/viewalltopicsupervisor');
          handleClose();
    ...
    >
      <Box className="adding-topic-form-modal">
        ...
        <form className="adding-topic-supervisor">
          <label htmlFor="topic-select-supervisor">Promotor:</label>
